  ...
  return {
    ...
    props: _objectSpread(_objectSpread({}, props), {}, {
      children: flatten(children).map(function (child) {
        return (
          // It does 2 things, decides if the element is of type object or text, and also flattens arrays.
...
function createTextElement(text) {
  return {
    type: 'TEXT_ONLY',
    // Elements are of type Object or TEXT_ONLY
    props: {
      nodeValue: text,
...
function createDom(fiber) {
  var dom = fiber.type == "TEXT_ONLY" ? document.createTextNode("") : document.createElement(fiber.type);
...
function updateDom(dom, prevProps, nextProps) {
  ...
  Object.keys(prevProps).filter(isEvent).filter(function (key) {
    ...
    //Remove event listeners before removing the node
